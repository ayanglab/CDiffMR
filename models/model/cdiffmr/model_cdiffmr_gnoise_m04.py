...
class CDiffMR(ModelBase):
    ...
    def feed_data(self, data):
        ...
        self.noise = torch.randn_like(self.x_start).to(self.device)

    def train_forward(self):

        self.x_t, self.x_recon = self.model_DM(x_start=self.x_start,
                                                  x_end=self.noise,
                                                  x_obs=self.x_obs,
                                                  mask=self.mask)
    ...
    def test(self):
        # self.model_DM.eval()
        with torch.no_grad():
            self.x_t, self.x_direct_recon, self.x_recon = self.model_EMA.sample(img=self.noise,
                                                                                x_obs=self.x_obs,
                                                                                mask=self.mask,
                                                                                batch_size=1,
                                                                                t=None)
