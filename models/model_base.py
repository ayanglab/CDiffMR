...
class ModelBase():
    ...
    # update for diffusion model
    def update_ema(self, decay=0.999):
        model_DM = self.get_bare_model(self.model_DM)
        model_DM_params = dict(model_DM.named_parameters())
        model_EMA_params = dict(self.model_EMA.named_parameters())
        for k in model_DM_params.keys():
            model_EMA_params[k].data.mul_(decay).add_(model_DM_params[k].data, alpha=1-decay)
